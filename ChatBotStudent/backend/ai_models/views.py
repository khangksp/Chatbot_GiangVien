...
def agent_status(request):
    ...
    """
    try:
        if chatbot_ai.agent_integration:
            stats = chatbot_ai.agent_integration.get_stats()
            return Response({
                'success': True,
                'agent_enabled': True,
                'stats': stats
            ...
            return Response({
                'success': True,
                'agent_enabled': False,
                'message': 'Agent mode not initialized, using legacy mode'
            })
...
@api_view(['POST'])
@permission_classes([AllowAny])
def switch_agent_mode(request):
    """
    Switch between agent and legacy mode
    Body: {"enable_agent": true/false}
    """
    try:
        enable_agent = request.data.get('enable_agent', True)
        
        if chatbot_ai.agent_integration:
            success = chatbot_ai.agent_integration.switch_mode(enable_agent)
            ...
            if success:
                ...
                return Response({
                    'success': False,
                    'message': 'Could not switch mode'
                }, status=400)
        else:
            return Response({
                'success': False,
                'message': 'Agent integration not initialized'
            }, status=400)
            
    except Exception as e:
        logger.error(f"❌ Switch agent mode error: {str(e)}")
        return Response({
            'success': False,
            'error': str(e)
        }, status=500)